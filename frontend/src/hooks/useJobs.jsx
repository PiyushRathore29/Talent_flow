/*
 * JOBS MANAGEMENT HOOK - useJobs.jsx
 *
 * JOBS CONTEXT EXPLANATION:
 * This hook provides comprehensive job management functionality including:
 * - Job CRUD operations (Create, Read, Update, Delete)
 * - Job flow/pipeline management with visual nodes and edges
 * - Candidate assignment to job stages
 * - Assessment integration with job stages
 * - Role-based job visibility (HR sees company jobs, candidates see all)
 *
 * DATA STRUCTURE:
 * - jobs: Object with job IDs as keys and job data as values
 * - jobOrder: Array of job IDs in display order
 * - Each job contains: basic info, flow nodes, flow edges, candidate assignments
 *
 * FLOW RECONSTRUCTION:
 * - Jobs can have saved flow data (nodes/edges) or reconstruct from database stages
 * - Flow data includes job nodes, stage nodes, and connecting edges
 * - Candidate nodes show applicants in each stage
 * - Assessment nodes show assessment status and completion
 */
...
// JOBS CONTEXT: Provides job management state and functions to all components
const JobsContext = createContext();

// HOOK EXPORT: Allows components to access jobs context
export const useJobs = () => useContext(JobsContext);

// FLOW RECONSTRUCTION HELPER:
// Reconstructs job flow data from saved flow data or database stages
const reconstructJobFlow = async (job) => {
  try {
    ...
    // STEP 1: Check for saved flow data (primary source of truth)
    // Saved flow data contains the complete visual representation
    const savedNodes = job.flowData?.nodes || [];
    const savedEdges = job.flowData?.edges || [];

    console.log(
      "📊 Saved flow data - nodes:",
      savedNodes.length,
      "edges:",
      savedEdges.length
    );
    console.log(
      "🔍 Saved nodes details:",
      savedNodes.map((n) => ({ id: n.id, type: n.type, stage: n.data?.stage }))
    );

    // STEP 2: If saved flow data exists, use it directly
    // This is the preferred method as it preserves user customizations
    ...
    console.log(
      "🔄 No saved flow data, reconstructing from database stages..."
    );
    ...
    console.log(
      "✅ Reconstructed from database - nodes:",
      allNodes.length,
      "edges:",
      edges.length
    );
...
// Manages job state and provides job operations to all child components
export const JobsProvider = ({ children }) => {
  // AUTHENTICATION: Get current user for role-based data access
  const { user } = useAuth();

  // JOB STATE MANAGEMENT:
  // jobs: Object storing all jobs with IDs as keys
  // jobOrder: Array of job IDs in display order
  ...
  // JOB LOADING EFFECT:
  // Loads jobs based on user role and company association
  useEffect(() => {
    const loadJobs = async () => {
      ...
      try {
        ...
        // STEP 2: Load jobs based on user role
        if (user.role === "hr" && user.companyId) {
          // HR users see only jobs from their company
          jobsList = await dbHelpers.getJobsByCompany(user.companyId);
        } else if (user.role === "candidate") {
          // Candidates see all jobs from all companies (for job applications)
        ...
        for (const jobId of Object.keys(jobsObj)) {
          try {
            const candidates = await dbHelpers.getCandidatesByJob(
              parseInt(jobId)
            );
            console.log(
              `🔍 Loading candidates for job ${jobId}:`,
              candidates.length,
              "candidates found"
            );
            ...
            const assessments = await dbHelpers.getAssessmentsByJob(
              parseInt(jobId)
            );
            ...
            if (jobsObj[jobId].nodes && jobsObj[jobId].nodes.length > 0) {
              console.log(
                `🧩 Job ${jobId} has ${jobsObj[jobId].nodes.length} nodes:`,
                jobsObj[jobId].nodes.map((n) => ({ id: n.id, type: n.type }))
              );

              jobsObj[jobId].nodes = await Promise.all(
                jobsObj[jobId].nodes.map(async (node) => {
                  if (node.type === "candidate") {
                    // Find candidates for this stage
                    const stageCandidates = candidates.filter(
                      (candidate) => candidate.currentStageId === node.id
                    );
                    console.log(
                      `📍 Stage "${node.id}" checking against candidates:`,
                      candidates.map((c) => c.currentStageId)
                    );
                    console.log(
                      `📍 Stage "${node.id}" matched ${stageCandidates.length} candidates:`,
                      stageCandidates.map((c) => c.name)
                    );
                  ...
                  if (node.type === "assessment") {
                    ...
                    const stageAssessment = assessments.find(
                      (assessment) => assessment.stageId === node.id
                    );
                    ...
                    if (stageAssessment) {
                      try {
                        const questions =
                          await dbHelpers.getQuestionsByAssessment(
                            stageAssessment.id
                          );
                        ...
                        candidatesWithAssessmentStatus = await Promise.all(
                          stageCandidates.map(async (candidate) => {
                            try {
                              const response =
                                await dbHelpers.getCandidateAssessmentResponse(
                                  stageAssessment.id,
                              ...
                              console.error(
                                `Failed to check assessment completion for candidate ${candidate.id}:`,
                                error
                              );
                              return {
                                ...candidate,
                        ...
                        console.error(
                          `Failed to load questions for assessment ${stageAssessment.id}:`,
                          error
                        );
                      }
                    }
  ...
  const updateJobFlow = useCallback(async (jobId, flowData) => {
    try {
      console.log(
        "🔄 updateJobFlow called with jobId:",
        jobId,
        "type:",
        typeof jobId
      );
      ...
      console.log(
        "🔍 Verification - Job flowData after update:",
        verifyJob?.flowData
      );
      console.log(
        "🔍 Verification - Job flowData nodes count:",
        verifyJob?.flowData?.nodes?.length || 0
      );
