...
import { FileText, Users, CheckCircle, Clock, Eye, AlertTriangle, RefreshCw } from "lucide-react";
...
import { AssessmentsService, CandidatesService } from "../../../lib/db/operations";
...
export const AssessmentResponsesPage = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const [responsesData, assessmentsData, candidatesData] = await Promise.all([AssessmentResponseService.getResponsesForCandidate(""),
      // Get all responses
      AssessmentsService.getAll(), CandidatesService.getAll()]);
      ...
      if (!allResponses || allResponses.length === 0) {
        ...
        const candidateResponses = await Promise.all(candidatesData.map(candidate => AssessmentResponseService.getResponsesForCandidate(candidate.id)));
  ...
  const handleExportResponse = useCallback(response => {
    ...
    const exportData = {
      ...
      response: {
        ...
        responses: response.responses.map(r => {
          const question = assessment.sections.flatMap(s => s.questions).find(q => q.id === r.questionId);
          return {
    ...
    a.href = url;
    a.download = `assessment-response-${candidate.name}-${new Date(response.submittedAt).toISOString().split("T")[0]}.json`;
  ...
  const handleUpdateStatus = useCallback(async (responseId, status) => {
    try {
      const updatedResponse = await AssessmentResponseService.updateResponse(responseId, {
        status
      });
      if (updatedResponse) {
        ...
        children: [/*#__PURE__*/_jsxs(Card, {
          ...
          children: [/*#__PURE__*/_jsxs(CardHeader, {
            ...
            children: [/*#__PURE__*/_jsx("div", {
              ...
              children: ["From ", candidates.length, " candidate", candidates.length !== 1 ? "s" : ""]
            })]
          })]
        }), /*#__PURE__*/_jsxs(Card, {
          style: {
        ...
        children: /*#__PURE__*/_jsxs(DialogContent, {
          ...
          children: [/*#__PURE__*/_jsxs(DialogHeader, {
            children: [/*#__PURE__*/_jsx(DialogTitle, {
              ...
              children: "Review the candidate's responses to the assessment questions"
            })]
          }), selectedResponse && /*#__PURE__*/_jsx(AssessmentResponseViewer, {
            response: selectedResponse,
            assessment: assessments.find(a => a.id === selectedResponse.assessmentId),
            candidate: candidates.find(c => c.id === selectedResponse.candidateId),
