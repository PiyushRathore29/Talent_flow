...
const JobFlowPage = () => {
  ...
  const handleNodesChange = useCallback(
    async (changes) => {
      ...
      // Check if any job nodes were moved (position changes) - only when drag ends
      const jobMoves = changes.filter(
        (change) =>
      ...
      // Check if any candidate nodes were moved between stages - only when drag ends
      const candidateMoves = changes.filter(
        (change) =>
          change.type === "position" &&
          change.dragging === false && // Only when drag ends
          nodes.find(
            (node) => node.id === change.id && node.type === "candidateNode"
          )
      );
  ...
  const handleCandidateStageMove = async (nodeId, newPosition) => {
    try {
        ...
        if (candidateNode && candidateNode.data?.candidates) {
          ...
          for (const candidate of candidateNode.data.candidates) {
            if (candidate.stage !== closestStage.id) {
              console.log(
                `🔄 [Flow] Moving candidate ${candidate.id} to stage ${closestStage.id}`
              );
      ...
      console.error(
        "❌ [Flow] Failed to move candidate between stages:",
        error
      );
    }
  };
  ...
  const fetchCandidates = async () => {
    try {
      ...
      // Use IndexedDB directly
      const data = await candidatesAPI.getAll({ jobId: jobId, pageSize: 100 });
      setCandidates(data.data || []);
    } catch (err) {
  ...
  useEffect(() => {
    const initializeData = async () => {
      setLoading(true);
      await Promise.all([fetchAllJobs(), fetchJob(), fetchCandidates()]);
      setLoading(false);
    };
  ...
  useEffect(() => {
    ...
    // Dynamic positioning based on stage content
    let currentY = 300;
    ...
    stages.forEach((stage, stageIndex) => {
      ...
      // Calculate dynamic Y position for this stage
      ...
      // Calculate height needed for this stage's content
      const candidateCount = stageCandidates.length;
  ...
  const handleStageChange = async (candidateId, newStage) => {
    try {
      // Get candidate details before the change for timeline
      ...
      // Refresh candidates while preserving node positions
  ...
  // New function to refresh candidates while preserving node positions
  const refreshCandidatesWithPositions = async () => {
    try {
      ...
      // Update existing nodes with new candidate data and dynamic positions
        ...
        return currentNodes.map((node) => {
          if (node.type === "candidateNode") {
            ...
            for (let i = 0; i < stageIndex; i++) {
              ...
              const prevStageCandidates = updatedCandidates.filter(
                (c) => c.stage === prevStage.id
              );
              const prevCandidateCount = prevStageCandidates.length;
          ...
          // Update stage title nodes positions too
          if (node.type === "stageTitleNode") {
            ...
            if (stageIndex !== -1) {
              ...
              return {
                ...node,
                position: { ...node.position, y: calculatedY - 50 },
              };
            }
          }
  ...
  const handleSimulateFlow = () => {
    alert(
      "Simulation feature coming soon! This will demonstrate candidate movement through the pipeline."
    );
  };
  ...
  return (
    <div className="h-screen bg-gray-50 dark:bg-black flex">
      {/* Jobs Sidebar */}
      <div
      ...
      <div className="flex-1 flex flex-col">
            ...
            <div className="flex justify-between items-center py-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Visual pipeline management • {candidates.length} candidates
                  total
                </p>
              </div>
        ...
        <div className="flex-1">
          ...
          >
            ...
            <Panel
              position="top-right"
              className="bg-white rounded-lg shadow-lg border p-4 space-y-3"
            >
              ...
              <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {stages.find((s) => s.id === selectedCandidate.stage)
                        ?.name || selectedCandidate.stage}
                    </p>
                  </div>
