...
const CandidateJobsBoard = () => {
  ...
  useEffect(() => {
    const loadJobs = async () => {
      ...
      try {
        ...
        const userApplications = await dbHelpers.getApplicationsByCandidate(
          user.id
        );
  ...
  const handleApplyToJob = async (jobId) => {
    try {
      ...
      const existingApplications = await dbHelpers.getApplicationsByCandidate(
        user.id
      );
      ...
      const appliedStage = jobStages.find(
        (stage) =>
          stage.name.toLowerCase().includes("applied") || stage.order === 1 // If no "applied" stage, use the first stage
      );

      if (!appliedStage) {
        console.error(
          "❌ No applied stage found for job:",
          jobId,
          "Available stages:",
          jobStages
        );
        alert(
          "Error: No application stage found for this job. Please contact HR."
        );
        return;
      }
      ...
      alert(
        `Successfully applied to ${job.details.title}! You can view your application status in your dashboard.`
      );
    } catch (error) {
  ...
  const locations = [
    ...new Set(
      availableJobs.map((job) => job.details.location).filter(Boolean)
    ),
  ];
  const jobTypes = [
    ...new Set(availableJobs.map((job) => job.details.type).filter(Boolean)),
  ];
  ...
  const filteredJobs = availableJobs.filter((job) => {
    ...
    const matchesLocation =
      locationFilter === "All" || job.details.location === locationFilter;
        ...
        <div className="max-w-screen-xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6 mb-8">
            ...
            <div className="mt-4 flex items-center justify-between text-sm text-gray-600">
              ...
              <span>{appliedJobIds.size} applications submitted</span>
            </div>
          </div>
