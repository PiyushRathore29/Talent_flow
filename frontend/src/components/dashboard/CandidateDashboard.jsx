...
const CandidateDashboard = () => {
    ...
    const loadApplications = async () => {
      ...
      try {
        ...
        for (const app of userApplications) {
          ...
          const statusDisplay = await getStatusDisplay(
            app.status,
            currentStageId,
            app.jobId
          );
  ...
  const getStatusDisplay = async (status, currentStageId, jobId) => {
    ...
    if (currentStageId && jobId) {
        ...
        if (
          ...
          try {
            ...
            const stageAssessment = assessments.find(
              (assessment) => assessment.stageId === currentStageId
            );
      ...
      if (stageId.includes("applied")) {
        ...
        stageId.includes("assessment") ||
        stageId.includes("test") ||
        stageId.includes("oa")
      ) {
        return {
          icon: <FileText className="w-4 h-4 text-indigo-500" />,
  ...
  const handleStartAssessment = async (jobId, stageId) => {
    try {
      ...
      const stageAssessment = assessments.find(
        (assessment) => assessment.stageId === stageId
      );
            ...
            {applications.length > 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead className="border-b">
                      <tr>
                        <th className="p-6 font-inter font-semibold text-primary-700">
                          Job Title
                        </th>
                        <th className="p-6 font-inter font-semibold text-primary-700">
                          Company
                      ...
                      {applications.map((app, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="p-6">
                              <span
                                className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-semibold ${statusDisplay.className}`}
                              >
                                {statusDisplay.icon}
                                {statusDisplay.text}
                              </span>
              ...
              <div className="bg-white rounded-lg shadow-md p-8 text-center">
                ...
                <p className="text-gray-500 mb-4">
                  Start your job search by browsing available opportunities.
                </p>
                <Link
                  to="/dashboard/candidate/jobs"
