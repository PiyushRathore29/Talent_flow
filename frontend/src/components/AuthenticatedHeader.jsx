...
const AuthenticatedHeader = () => {
  ...
  const getNavigationLinks = () => {
    if (user?.role === 'hr') {
      return [
        { path: '/dashboard/employer', label: 'Dashboard', icon: <BarChart className="w-4 h-4" /> },
        { path: '/candidates', label: 'Candidates', icon: <Users className="w-4 h-4" /> }
      ];
    } else {
      return [
        { path: '/dashboard/candidate', label: 'Dashboard', icon: <BarChart className="w-4 h-4" /> },
        { path: '/jobs', label: 'Browse Jobs', icon: <Briefcase className="w-4 h-4" /> }
      ];
    }
  };
